...
tags: []
subtitle: ''
summary: 'This post is about Naive Bayes Classifier; currently just implementation in R, hopefully will soon update to include the theory of the method'
authors: []
lastmod: '2021-04-25T14:49:04-04:00'
featured: no
image:
  caption: ''
  focal_point: ''
...
social_network_ads <- fread("C:/Users/diji_/Desktop/Data Science/Machine Learning A-Z (Codes and Datasets)/Part 3 - Classification/Section 18 - Naive Bayes/R/Social_Network_Ads.csv")
```
...
naive_bayes_classifier <- naiveBayes(as.factor(Purchased) ~ Age + EstimatedSalary, data = training_set)
```

```{r}
# Make predictions on the test set
test_predictions <- predict(naive_bayes_classifier, test_set)
```

```{r}
# Create the confusion matrix
table(test_predictions, as.factor(test_set$Purchased))
```
...
```{r}
y_grid_pred <- predict(naive_bayes_classifier, the_grid)
```
...
ggplot() +
  ...
  xlab("Salary") +
  ylab("Age") +
  ggtitle("Naive Bayes used to determine purchase of an item based on age and salary.") +
  theme_classic() +
